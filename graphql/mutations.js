...
const addComment = {
  ...
  async resolve(_, { postId, comment }, { verifiedUser }) {
    ...
    // Llama a la función de manejo del webhook
    await handleWebhook(newComment);
...
const handleWebhook = async (comment) => {
  try {
    console.log(`Nuevo comentario: ${comment.comment}`);

    // Configura la URL de tu servidor o servicio de webhook
    const webhookURL = 'http://localhost:3000';

    // Envía el comentario al servidor de webhook
    await axios.post(`${webhookURL}/webhook/comment`, {
      comment: comment.comment,
      // Otros datos que quieras enviar al webhook
    });
